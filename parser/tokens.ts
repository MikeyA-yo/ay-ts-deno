...
export class TokenGen {
  ...
  next(): void {
    const currentLineToken = this.tokenizeLine(this.lines[this.currentLine]);
  ...
  peek(steps?:number):Token | undefined{
    let token;
    if(steps){
  ...
  getRemainingToken() {
    const tokensLeft: Token[] = this.tokenizeLine(
      this.lines[this.currentLine]
    ).slice(this.currentTokenNo + 1);
    const linesLeft = this.lines.slice(this.currentLine + 1);
    for (let i = 0; i < linesLeft.length; i++) {
      const lineTokens = this.tokenizeLine(linesLeft[i]);
